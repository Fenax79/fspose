...
def train(config, train_loader, model, criterion, optimizer, epoch,
          output_dir, tb_log_dir, writer_dict, mobilenet=False):
...
def validate(config, val_loader, val_dataset, model, criterion, output_dir,
             tb_log_dir, writer_dict=None, mobilenet=False, input_heatmap_dir=None, save_accuracy_folder=None):
    batch_time = AverageMeter()
    losses = AverageMeter()
    acc = AverageMeter()
    ...
    with torch.no_grad():
      ...
      if not use_warping:
        for i, (input, target, target_weight, meta) in enumerate(val_loader):
            ...
            hoge = flop_count(model, (input.cuda(non_blocking=True), ))
            '''
            ...
            if input_heatmap_dir is None:
                # compute output
                #start_ = time.time()
                outputs = model(input)
                #end_ = time.time()
                #csv_data.append(["w32 time", str(end_-start_)])
                ...
                outputs = torch.zeros(target.shape, dtype=torch.float32)
                
                for j,image_file in enumerate(meta['image']):
                    parent = os.path.join(input_heatmap_dir, image_file.split('/')[-3], image_file.split('/')[-2], 
                        os.path.splitext(image_file.split('/')[-1])[0])
                    outputs[j] = torch.load(os.path.join(parent, str(int(meta['track_id'][j])) + ".pt"))

                #outputs[:,:,:,56-7:] = 0
                #outputs[:,:,:,:7] = 0
                output = outputs.cuda(non_blocking=True)
            ...
            each_acc, avg_acc, cnt, pred, target_keys = accuracy(output.cpu().numpy(),
                                             target.cpu().numpy(), output_target_keys=True)
            acc.update(avg_acc, cnt)
            if save_accuracy_folder is not None:
                save_accuracy(save_accuracy_folder, each_acc, meta['image'], meta["track_id"], output=output.cpu().numpy(), target=target.cpu().numpy())

            # for j, target_key in enumerate(target_keys):
            #    target_gt_json[meta['image'][j]] = target_key.tolist()
        ...
        #with open("target_gt.json", "w") as f:
        #    json.dump(target_gt_json, f)

        #import csv 
        #with open("w32_time.csv", "w") as f:
        #    writer = csv.writer(f)
...
def create_heatmap_and_save(config, data_loader, dataset, model, criterion, output_dir,
             tb_log_dir, heatmap_dir, writer_dict=None, mobilenet=False):
    batch_time = AverageMeter()
    # switch to evaluate mode
    model.eval()
    ...
    assert not use_warping and not use_gt_input
    with torch.no_grad():
        end = time.time()
        for i, (input, target, target_weight, meta) in enumerate(data_loader):
            ...
            for i,image_file in enumerate(meta['image']):
                parent = os.path.join(heatmap_dir, image_file.split('/')[-3], image_file.split('/')[-2], os.path.splitext(image_file.split('/')[-1])[0])

                #if config.DATASET.IS_POSETRACK18:
                #    parent = parent.parent

                os.makedirs(parent, exist_ok=True)
                torch.save(outputs[i], os.path.join(parent, str(int(meta['track_id'][i])) + ".pt"))
                outputs[i]
                print(parent)
...
def create_rgb_and_save(config, data_loader, dataset, output_dir,
             tb_log_dir, rgb_dir, writer_dict=None):
    batch_time = AverageMeter()
    ...
    for i, (input, target, target_weight, meta) in enumerate(data_loader):
        ...
        for i,image_file in enumerate(meta['image']):
            parent = os.path.join(rgb_dir, image_file.split('/')[-3], image_file.split('/')[-2], os.path.splitext(image_file.split('/')[-1])[0])
            os.makedirs(parent, exist_ok=True)
            output_filename = os.path.join(parent, str(int(meta['track_id'][i])) + ".png")
            rgb_numpy = input[i].to('cpu').detach().numpy().copy()
            bgr_numpy = cv2.cvtColor(rgb_numpy, cv2.COLOR_RGB2BGR)
            cv2.imwrite(output_filename, bgr_numpy)
            # torch.save(outputs[i], output_filename)
            # outputs[i]
            # print(parent)
