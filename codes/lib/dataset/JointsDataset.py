...
class JointsDataset(Dataset):
    def __init__(self, cfg, root, image_set, is_train, transform=None):
        ...
        self.save_dataset_img = cfg['SAVE_DATASET_IMG']
        self.half_body = cfg['HALF_BODY']
        ...
        self.db = []
        # self.use_warping_train = True
        print("self.use_warping_train: ", self.use_warping_train)
        print("self.use_warping_test", self.use_warping_test)
    ...
    def __getitem__(self, idx):
        ...
        if self.data_format == 'zip':
            ...
            if (self.is_train and self.use_warping_train) or (not self.is_train and self.use_warping_test):
               ...
               if not self.is_posetrack18:
                  new_sup_image_file = image_file.replace(prev_nm, str(sup_idx).zfill(8) + '.jpg')
               else:
                  new_sup_image_file = image_file.replace(prev_nm, str(sup_idx).zfill(6) + '.jpg')
        ...
        if self.color_rgb:
            ...
            if (self.is_train and self.use_warping_train) or (not self.is_train and self.use_warping_test):
               data_numpy_sup = cv2.cvtColor(data_numpy_sup, cv2.COLOR_BGR2RGB)
        ...
        r = 0
        '''
        if s[1] / s[0] >=  self.image_size[1] / self.image_size[0]:
            s[0] = s[1] *  self.image_size[0] / self.image_size[1]
        else:
            s[1] = s[0] *  self.image_size[1] / self.image_size[0]
        '''
        if self.is_train:
            if (self.half_body and np.sum(joints_vis[:, 0]) > self.num_joints_half_body
        ...
        if self.save_dataset_img:
            parent = os.path.join(self.output_path, image_file.split('/')[-3], image_file.split('/')[-2], os.path.splitext(image_file.split('/')[-1])[0])
            os.makedirs(parent, exist_ok=True)
            filepath = os.path.join(parent, str(id_) + ".jpg")
            cv2.imwrite(filepath, cv2.cvtColor(input, cv2.COLOR_BGR2RGB))


        '''
        cv2.imshow("org", cv2.cvtColor(data_numpy, cv2.COLOR_BGR2RGB))
        cv2.imshow("transed", cv2.cvtColor(input, cv2.COLOR_BGR2RGB))
        cv2.waitKey(10)
        print("transed")
        '''
        ...
        '''
        if ref_idx - sup_idx >= 50:
            cv2.imwrite("/home/storm/temp/input_sup.jpg", cv2.cvtColor(input_sup, cv2.COLOR_BGR2RGB))
            cv2.imwrite("/home/storm/temp/input.jpg", cv2.cvtColor(input, cv2.COLOR_BGR2RGB))
            cv2.imwrite("/home/storm/temp/data_numpy_sup.jpg", cv2.cvtColor(data_numpy_sup, cv2.COLOR_BGR2RGB))
            cv2.imwrite("/home/storm/temp/data_numpy.jpg", cv2.cvtColor(data_numpy, cv2.COLOR_BGR2RGB))
        '''
    ...
    def generate_target(self, joints, joints_vis, heatmap_height=None, heatmap_width=None):
        '''
        ...
        if self.target_type == 'gaussian':
            if heatmap_height is None or heatmap_width is None:
