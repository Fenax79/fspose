...
from dataset.JointsDataset_same_person import JointsDatasetSamePerson
...
class PoseTrackDatasetSamePerson(JointsDatasetSamePerson):
    '''
    "keypoints": {
        0: "nose",
        1: "left_eye",
    ...
    def __init__(self, cfg, root, image_set, is_train, transform=None):
        ...
        self.image_index_to_db_indexes = {}  # int to list
           ...
           if self.num_labeled_frames_per_video <0:
             ...
             if self.pseudo_gt_train:
                if self.num_labeled_videos != 250:
                    return self.json_dir + 'posetrack_train_N'+str(self.num_labeled_frames_per_video)+'_per_video_V'+str(self.num_labeled_videos)+'_videos_wPseudoGT.json'
                else:
    ...
    def _get_db(self):
        assert self.is_train or self.use_gt_bbox
    ...
    def _load_coco_keypoint_annotations(self):
        ...
        gt_db = []
        self.image_index_to_db_indexes = {}
        for index in self.image_set_index:
            rec = self._load_coco_keypoint_annotation_kernal(index)
            start = len(gt_db)
            gt_db.extend(rec)
            end = len(gt_db)
            self.image_index_to_db_indexes[index] = [i for i in range(start, end)]
