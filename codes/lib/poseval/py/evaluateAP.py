...
from poseval.py.eval_helpers import assignGTmulti, computeRPC, Joint, VOCap
...
def computeMetrics(scoresAll, labelsAll, nGTall):
    ...
    for j in range(nGTall.shape[0]):
        ...
        #print(str(j), "labels", list(labels).count(1), len(labels), "ap", apAll[j],
        #    "precision", precision, "recall", recall, "nGT", nGT)
