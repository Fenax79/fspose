...
# pretrained_coco_model = root_dir + 'pretrained_models/pose_hrnet_w32_384x288.pth'
pretrained_coco_model = root_dir + 'pretrained_models/pose_hrnet_w32_256x192.pth'
  ...
  for V in V_list: ## number of labeled videos
    for N in N_list: ## number of labeled frames per video
        ...
        if jj == 0:
           ...
           command = cur_python+' '+working_dir+'/tools/train.py --cfg experiments/posetrack/hrnet/w32_256x192_adam_lr1e-4_PoseWarper_train.yaml OUTPUT_DIR '+out_dir+' LOG_DIR '+log_dir+' DATASET.NUM_LABELED_VIDEOS '+str(V)+' DATASET.NUM_LABELED_FRAMES_PER_VIDEO '+str(N)+ ' DATASET.JSON_DIR '+json_dir +' DATASET.IMG_DIR '+img_dir +' MODEL.PRETRAINED ' +pretrained_model+ ' MODEL.EVALUATE False PRINT_FREQ '+str(PF)+epoch_sfx
           print(command)
           os.system(command)

        #### temporal pose aggregation
        annot_sfx = 'val_withJson/val/'
        sfx = 'posetrack/pose_hrnet/w32_256x192_adam_lr1e-4_PoseWarper_train/final_state.pth'
        ...
        if rr == 0: ## video pose propagation evaluation
           ...
           if jj == 0:
              command = cur_python+' '+working_dir+'/tools/train.py --cfg experiments/posetrack/hrnet/w32_256x192_adam_lr1e-4_pseudo_gt_train.yaml OUTPUT_DIR '+out_dir+' LOG_DIR '+log_dir+' DATASET.NUM_LABELED_VIDEOS '+str(V)+' DATASET.NUM_LABELED_FRAMES_PER_VIDEO '+str(N)+ ' DATASET.JSON_DIR '+json_dir +' DATASET.IMG_DIR '+img_dir +' MODEL.PRETRAINED ' +pretrained_coco_model+ ' MODEL.EVALUATE False PRINT_FREQ '+str(PF)
              print(command)
              os.system(command)

           ##### inference
           annot_sfx = 'val_withJson/val/'
           sfx = 'posetrack/pose_hrnet/w32_256x192_adam_lr1e-4_pseudo_gt_train/final_state.pth'
           inference_model_path = out_dir + sfx
           if jj == 1:
